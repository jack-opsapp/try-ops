...
export function FeatureCard({ icon, title, description, forLine, bullets, children }: FeatureCardProps) {
  return (
    <motion.div
      className="bg-ops-card border border-ops-border rounded-ops-card p-8 transition-all duration-300 hover:border-ops-accent hover:-translate-y-1"
      {...fadeInUp}
    >
      {icon && <div className="mb-4">{icon}</div>}
      <h3 className="font-mohave font-medium text-[18px] uppercase text-white mb-4">
        {title}
      </h3>
