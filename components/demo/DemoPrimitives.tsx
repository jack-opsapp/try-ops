...
export function SamplePhoto({ src, alt, className = '', priority = false, sizes = '(max-width: 600px) 100vw, 520px' }: { src: string; alt: string; className?: string; priority?: boolean; sizes?: string }) { const [failed, setFailed] = useState(false); return failed ? <div className={`${ui.photoFallback} ${className}`}>Sample photo unavailable. You can continue.</div> : <Image className={`${ui.photo} ${className}`} src={src} alt={alt} width={1200} height={900} sizes={sizes} priority={priority} onError={()=>setFailed(true)} /> }
