...
function notificationBody(text: string) {
  const numeric = /^(?:(?:EST|INV)-\d+|\$[\d,]+(?:\.\d{2})?|\d{1,2}:\d{2}(?: [AP]M)?)$/
  return text.split(/((?:EST|INV)-\d+|\$[\d,]+(?:\.\d{2})?|\d{1,2}:\d{2}(?: [AP]M)?)/g)
...
export function DemoNotification({ notification, onDismiss, reduced = false }: DemoNotificationProps) {
