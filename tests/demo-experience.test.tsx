...
function reachOperatorActivity(name = 'Pete') { reachProject(); assignInstallationCrew(name); click('See the completed work') }
function reachComposer() { click('Choose Crew'); click('Complete'); click('Post a photo update') }
...
describe('the visitor sample job', () => {
  ...
  it('discards a cancelled crew choice and attributes incoming work only to the selected person', () => {
    ...
    expect(screen.queryByRole('button', { name: 'See the completed work' })).toBeNull()
    click('Assign team to this task')
    expect(screen.getByRole('button', { name: 'Select Pete' }).getAttribute('aria-pressed')).toBe('false')
    click('Select Nick'); click('Done'); click('See the completed work')
    first.unmount(); render(<DemoExperience />)
