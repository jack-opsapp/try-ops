...
describe('estimate beta and the optional custom tool', () => {
  ...
  it('updates drawing dimensions and sample quantities without changing the prepared quote', () => {
    ...
    for (const [width, area, boards] of [[12, 144, 20], [16, 192, 27], [20, 240, 33]]) {
      const widthLabel = `${width}' 0"`
      click(widthLabel)
      expect(currentState().deckWidth).toBe(width)
      const drawing = screen.getByRole('img', { name: `${width} by 12 foot deck plan` })
      const rectangles = Array.from(drawing.querySelectorAll('rect'))
      shapes.add(rectangles.map(rectangle => `${rectangle.getAttribute('x')}:${rectangle.getAttribute('width')}`).join('|'))
      expect(drawing.querySelector(`rect[width="${width * 9}"]`)).not.toBeNull()
      expect(screen.getByText(`${area} sq ft`)).toBeTruthy()
      expect(screen.getByText('Sample board order').parentElement?.textContent).toBe(`Sample board order${boards}16 ft boards`)
      expect(screen.getByRole('button', { name: widthLabel }).getAttribute('aria-pressed')).toBe('true')
    }
    expect(shapes.size).toBe(3)
    fireEvent.click(screen.getByText('Takeoff assumptions'))
    expect(screen.getByText(/Fixed 12 ft depth/)).toBeTruthy()
    expect(screen.getByText('This preview shows one dimension change. It does not change your prepared estimate.')).toBeTruthy()
  ...
  it('supports Escape and restores keyboard focus to the custom tool trigger', () => {
    render(<VisitHarness initial={preparedEstimate()} />)
    click('Explore custom tool')
    expect(document.activeElement).toBe(screen.getByRole('heading', { name: SAMPLE.project }))
